...
export const markHostStarted = async (req: Request, res: Response) => {
  try {
    const { bookingId } = req.params
    const booking = await prisma.booking.findFirst({ where: { id: bookingId, therapist: { userId: req.currentUser!.userId } } })
    if (!booking) return res.status(404).json({ message: 'Booking not found' })
...
export const getSignature = async (req: Request, res: Response) => {
  try {
    ...
    if (req.currentUser!.role === Role.PARENT && !booking.hostStarted) {
      return res.status(403).json({ message: 'Host has not started the meeting yet' })
    }

    const role: 0 | 1 = req.currentUser!.role === Role.THERAPIST ? 1 : 0
